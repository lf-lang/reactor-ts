  ...
  foo = new class extends InPort<number> {
    constructor (container: Reactor) {
      ...
      test('receive runtime object twice', () => {
        ...
        }).toThrowError(`Can only establish link to runtime once. Name: ${String(this._getFullyQualifiedName())}`);
      });
      test('make port writable with invalid key', () => {
        expect(() => {
          this.asWritable(Symbol());
        }).toThrowError(`Referenced port is out of scope: ${String(this._getFullyQualifiedName())}`);
      });
