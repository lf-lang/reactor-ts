...
describe("time helper functions", function () {
  ...
  const twoHundredFiftyMillMS: TimeValue = TimeValue.withUnits(
    250000000,
    TimeUnit.msec
  );
  const fiveHundredMilNS: TimeValue = TimeValue.withUnits(
    500000000,
    TimeUnit.nsec
  );
  ...
  const fortyTwoDaysAsMsec: TimeValue = TimeValue.msec(
    42 * 24 * 60 * 60 * 1000
  );
  ...
  it("borrow a second", function () {
    expect(
      TimeValue.secsAndNs(2, 200000).subtract(TimeValue.secsAndNs(1, 300000))
    ).toEqual(TimeValue.secsAndNs(0, 200000 - 300000 + 1000000000));
    expect(TimeValue.secsAndNs(7, 3000).subtract(fiveSFiveUS)).toEqual(
      TimeValue.secsAndNs(7 - 5 - 1, 3000 - 5000 + 1000000000)
    );
    expect(TimeValue.secsAndNs(7, 6000).subtract(fiveSFiveUS)).toEqual(
      TimeValue.secsAndNs(7 - 5, 6000 - 5000)
    );
  ...
  it("multiply time value", function () {
    ...
    expect(TimeValue.secsAndNs(1, 500000000).multiply(2)).toEqual(
      TimeValue.secs(3)
    );
  });
...
describe("add time value", function () {
  ...
  it("converting to string", function () {
    ...
    expect(TimeValue.secsAndNs(5, 5000).toString()).toEqual(
      "(5 secs; 5000 nsecs)"
    );
    expect(TimeValue.secsAndNs(250000, 0).toString()).toEqual(
      "(250000 secs; 0 nsecs)"
    );
  ...
  it("time value equality", function () {
    ...
    expect(
      aboutTenYears.isEqualTo(TimeValue.secsAndNs(10 * 365 * 24 * 60 * 60, 0))
    );
    ...
    expect(
      sevenPointFiveBillNS.isEqualTo(sevenPointFiveMillUsecs)
    ).toBeTruthy();
  ...
  it("create tags", function () {
    ...
    expect(() => {
      const tiFiveSecondsMinus1: Tag = new Tag(fiveSeconds, -1);
    }).toThrowError("Microstep must be positive.");
    expect(() => {
      const tiFiveSeconds1Point1: Tag = new Tag(fiveSeconds, 1.1);
    }).toThrowError("Microstep must be integer.");
  });
  ...
  it("get a later tag", function () {
    expect(
      new Tag(fiveHundredMilNS, 0)
        .getLaterTag(fortyTwoDays)
        .isSimultaneousWith(
          new Tag(TimeValue.secsAndNs(42 * 24 * 60 * 60, 500000000), 0)
        )
    ).toBeTruthy();
    expect(
      new Tag(fiveHundredMilNS, 0)
        .getLaterTag(fiveHundredMilNS)
        .isSimultaneousWith(new Tag(TimeValue.secs(1), 0))
    ).toBeTruthy();
    expect(
   ...
   */
  it("compare tags", function () {
    expect(tiFiveSeconds0.getTimeDifference(tiFiveSeconds0)).toEqual(
      TimeValue.secs(0)
    );
    expect(tiFiveSeconds0.getTimeDifference(tiFiveSeconds1)).toEqual(
      TimeValue.secs(0)
    );
  ...
  it("compare time values", function () {
    expect(
      TimeValue.secsAndNs(10, 10).isEarlierThan(TimeValue.secsAndNs(11, 1))
    ).toEqual(true);
    expect(
      TimeValue.secsAndNs(0, 999999998).isEarlierThan(
        TimeValue.secsAndNs(0, 999999999)
      )
    ).toEqual(true);

    expect(
      TimeValue.secsAndNs(0, 999999999).isLaterThan(
        TimeValue.secsAndNs(0, 999999998)
      )
    ).toEqual(true);
    expect(
      TimeValue.secsAndNs(11, 1).isLaterThan(TimeValue.secsAndNs(10, 10))
    ).toEqual(true);
  });
  ...
  it("convert to binary", function () {
    ...
    expect(TimeValue.fromBinary(threeHundredUS.toBinary())).toEqual(
      threeHundredUS
    );
    expect(TimeValue.fromBinary(sevenPointFiveBillNS.toBinary())).toEqual(
      sevenPointFiveBillNS
    );
    expect(TimeValue.fromBinary(twoHundredFiftyMillMS.toBinary())).toEqual(
      twoHundredFiftyMillMS
    );
    expect(TimeValue.fromBinary(fiveSFiveUS.toBinary())).toEqual(fiveSFiveUS);
    expect(TimeValue.fromBinary(oneThousandMS.toBinary())).toEqual(
      oneThousandMS
    );
  });
  ...
  it("convert to/from binary with Tag methods", function () {
    ...
    expect(Tag.fromBinary(threeHundredUSTag.toBinary())).toEqual(
      threeHundredUSTag
    );
    expect(Tag.fromBinary(sevenPointFiveBillNSTag.toBinary())).toEqual(
      sevenPointFiveBillNSTag
    );
    expect(Tag.fromBinary(twoHundredFiftyMillMSTag.toBinary())).toEqual(
      twoHundredFiftyMillMSTag
    );
    expect(Tag.fromBinary(fiveSFiveUSTag.toBinary())).toEqual(fiveSFiveUSTag);
    expect(Tag.fromBinary(oneThousandMSTag.toBinary())).toEqual(
      oneThousandMSTag
    );
  });
});
