        ...
        try {
          ...
          while (
            util
              ...
              .isEarlierThan(
                initialElapsedTime.add(TimeValue.withUnits(1, TimeUnit.sec))
              )
          );
        } finally {
          if (tmp) {
...
class ReactorWithAction extends App {
  ...
  t = new Timer(
    this,
    TimeValue.withUnits(1, TimeUnit.msec),
    TimeValue.withUnits(1, TimeUnit.sec)
  );
...
describe("Testing Reactions", function () {
  ...
  it("Manually call reactions", () => {
    const app = new testApp(
      "testApp",
      TimeValue.withUnits(5000, TimeUnit.msec),
      () => {},
      fail
    );
