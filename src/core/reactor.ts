...
export abstract class Reactor extends Component {
  ...
  protected _dependencyGraph = new PrecedenceGraph<
    Port<Present> | Reaction<Variable[]>
  >();
  ...
  private _recordDeps<T extends Variable[]>(reaction: Reaction<T>): void {
    ...
    const sources = new Set<Port<Present>>();
    const effects = new Set<Port<Present>>();
  ...
  protected _getPrecedenceGraph(
    depth = -1
  ): PrecedenceGraph<Port<Present> | Reaction<Variable[]>> {
    const graph = new PrecedenceGraph<Port<Present> | Reaction<Variable[]>>();
  ...
  protected _getCausalityInterface(): PrecedenceGraph<Port<Present>> {
    ...
    const search = (
      ...
      nodes: Set<Port<Present> | Reaction<Variable[]>>
    ): void => {
...
export interface Runtime {
  ...
  schedule: (e: TaggedEvent<Present>) => void;
...
export class App extends Reactor {
  ...
  private readonly __runtime: Runtime = new (class implements Runtime {
     ...
     */
    public schedule(e: TaggedEvent<Present>): void {
